...
public class End2EndHttpsTest {
    ...
    void testGatewayCustomRouter(TestHttpMethod httpMethod, Vertx vertx, VertxTestContext vertxTestContext) {
        WebClient webClient = WebClient.create(vertx, new WebClientOptions().setSsl(true).setVerifyHost(false).setTrustAll(true));
    ...
    void testSSERequest(Vertx vertx, VertxTestContext vertxTestContext) {
        HttpClient httpClient = vertx.createHttpClient(new HttpClientOptions().setSsl(true).setVerifyHost(false).setTrustAll(true));
    ...
    void testWebsocketRequest(Vertx vertx, VertxTestContext vertxTestContext) {
        WebSocketClient webSocketClient = vertx.createWebSocketClient(new WebSocketClientOptions().setSsl(true).setVerifyHost(false).setTrustAll(true));
        ...
        }).listen(0).compose(httpServer -> connectToGateway(vertx, httpServer.actualPort(), "ws-service", gatewayRegisterPort, true)).onComplete(vertxTestContext.succeeding(httpServer -> {
