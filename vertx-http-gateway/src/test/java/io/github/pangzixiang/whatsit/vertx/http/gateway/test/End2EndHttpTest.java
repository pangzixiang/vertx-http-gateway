...
public class End2EndHttpTest {
    ...
    public void setUp(Vertx vertx, VertxTestContext vertxTestContext) {
        ...
        getewayRegisterCustomRouter = Router.router(vertx);
    ...
    @Test
    @Order(1)
    void testGetConnectors(Vertx vertx, VertxTestContext vertxTestContext) {
        WebClient webClient = WebClient.create(vertx);
        gatewayCustomRouter.get("/connectors").handler(routingContext -> routingContext.response().end(Json.encode(VertxHttpGatewayContext.getInstance(vertx).getConnectorServiceDetails())));
        webClient.get(gatewayProxyPort, "localhost", "/connectors").send().onComplete(vertxTestContext.succeeding(bufferHttpResponse -> {
            Assertions.assertEquals(200, bufferHttpResponse.statusCode());
            JsonArray jsonArray = bufferHttpResponse.bodyAsJsonArray();
            Assertions.assertEquals(1, jsonArray.size());
            Assertions.assertEquals(SERVICE_NAME, jsonArray.getJsonObject(0).getString("serviceName"));
            vertxTestContext.completeNow();
        }));
    }
    ...
    @ParameterizedTest
    @EnumSource(value = TestHttpMethod.class)
    void testGatewayRegisterCustomRouter(TestHttpMethod httpMethod, Vertx vertx, VertxTestContext vertxTestContext) {
        WebClient webClient = WebClient.create(vertx);
        getewayRegisterCustomRouter.route(HttpMethod.valueOf(httpMethod.name()), "/" + httpMethod).handler(routingContext -> routingContext.response().putHeader("t1", "test").end("ok"));
        webClient.request(HttpMethod.valueOf(httpMethod.name()), gatewayRegisterPort, "localhost", "/" + httpMethod).send().onComplete(vertxTestContext.succeeding(bufferHttpResponse -> {
            Assertions.assertEquals(200, bufferHttpResponse.statusCode());
            Assertions.assertEquals("ok", bufferHttpResponse.bodyAsString());
            Assertions.assertEquals("test", bufferHttpResponse.getHeader("t1"));
            vertxTestContext.completeNow();
    ...
    @Test
    void testGatewayConnectorTimeout(Vertx vertx, VertxTestContext vertxTestContext) {
        WebClient webClient = WebClient.create(vertx);
        testServiceRouter.get("/" + SERVICE_NAME + "/timeout").handler(rc -> {
        ...
        webClient.get(gatewayProxyPort, "localhost", "/" + SERVICE_NAME + "/timeout").send().onComplete(vertxTestContext.succeeding(bufferHttpResponse -> {
            Assertions.assertEquals(504, bufferHttpResponse.statusCode());
            vertxTestContext.completeNow();
        }));
    }
    ...
    @Test
    void testWebsocketRequest(Vertx vertx, VertxTestContext vertxTestContext) {
        WebSocketClient webSocketClient = vertx.createWebSocketClient();
        vertx.createHttpServer().webSocketHandler(serverWebSocket -> {
            if (serverWebSocket.path().equals("/ws-service/ws")) {
                serverWebSocket.handler(buffer -> {
                    log.info("Server received ws message {}", buffer);
                    serverWebSocket.writeBinaryMessage(buffer);
                });
            } else {
                serverWebSocket.reject();
            }
        }).listen(0).compose(httpServer -> connectToGateway(vertx, httpServer.actualPort(), "ws-service", gatewayRegisterPort)).onComplete(vertxTestContext.succeeding(httpServer -> {
            WebSocketConnectOptions webSocketConnectOptions = new WebSocketConnectOptions()
                    .setHost("localhost")
                    .setPort(gatewayProxyPort)
                    .setURI("/ws-service/ws");
            webSocketClient.connect(webSocketConnectOptions).onComplete(vertxTestContext.succeeding(webSocket -> {
    ...
    private Future<VertxHttpGatewayConnector> setUpTestServiceAndConnectGateway(Vertx vertx, Router router) {
        ...
        return vertx.createHttpServer().requestHandler(router).listen(0).compose(httpServer -> {
            ...
            return connectToGateway(vertx, httpServer.actualPort(), SERVICE_NAME, gatewayRegisterPort);
        });
    }
